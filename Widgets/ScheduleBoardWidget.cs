...
/// </summary>
/// <remarks>
/// 这是第一个迁移到自绘框架的业务控件。相比原来的 XAML 版本，它拿到了：
/// 圆角外真透明（不再有窗口底色露白）、抗锯齿圆弧、真正的窗口投影，
/// 以及完全自由的尺寸与折叠形态。
/// </remarks>
public sealed class ScheduleBoardWidget : WidgetWindow
{
    // ---- 逻辑尺寸（物理尺寸由 DPI 缩放算出）----
    ...
    public ScheduleBoardWidget()
        : base((int)(CardWidth + (ShadowMargin * 2)), (int)(CardHeight + (ShadowMargin * 2)))
    {
        PositionChanged += OnPositionChanged;
        ScheduleActivated += OnScheduleActivated;
        NewScheduleRequested += OnNewScheduleRequested;

        App.Data.Changed += OnDataChanged;
        Refresh(App.Data.Schedules);
    }

    /// <summary>拖动结束后把新位置写回设置，下次启动仍在原处。</summary>
    private void OnPositionChanged(double x, double y)
    {
        var settings = App.Settings.Current.ScheduleBoard;
        settings.X = x;
        settings.Y = y;
    }

    /// <summary>点击条目 → 打开独立日程详情窗口。</summary>
    private void OnScheduleActivated(ScheduleItem item) => Views.Dialogs.ScheduleDetailWindow.Open(item);

    /// <summary>
    /// 快捷新建日程（FR-5.3）：先弹对话框填写，点「创建」才落库。
    /// </summary>
    /// <remarks>
    /// 对话框必须挂在主窗口的 XamlRoot 上，主界面最小化时它无处可显示，
    /// 表现成「点了新建没反应」。这里先把主窗口恢复出来再弹。
    /// </remarks>
    private void OnNewScheduleRequested()
    {
        ...
        var dialog = new Views.Dialogs.ScheduleEditDialog(item, isNew: true) { XamlRoot = xamlRoot };

        _ = ShowNewScheduleDialogAsync(dialog, item);
    }

    private async Task ShowNewScheduleDialogAsync(ContentDialog dialog, ScheduleItem item)
    {
    ...
    private void OnDataChanged(object? sender, EventArgs e) => Refresh(App.Data.Schedules);
    ...
    /// <summary>请求打开某条日程的详情。</summary>
    public event Action<ScheduleItem>? ScheduleActivated;

    /// <summary>请求新建日程。</summary>
    public event Action? NewScheduleRequested;
    ...
    /// <summary>应用配色（基类 ApplyTheme 转发到这里）。</summary>
    ...
    public void Refresh(IEnumerable<ScheduleItem> schedules)
    {
        _items.Clear();
        _items.AddRange(schedules
    ...
    {
        var titleY = card.Top + 30f;
        WidgetPainting.DrawText(canvas, App.Loc["ScheduleTitle"], card.Left + 16f, titleY,
    ...
    private void DrawList(SKCanvas canvas, SKRect card)
    {
        if (_items.Count == 0)
        {
            WidgetPainting.DrawText(canvas, App.Loc["ScheduleBoardEmpty"],
        ...
        for (var i = 0; i < _items.Count; i++)
        {
            ...
            var textColor = item.IsCompleted
                ? WidgetTheme.Fade(_theme.TextPrimary, 0x66)
                : _theme.TextPrimary;

            var available = rowRect.Width - 24f;

            WidgetPainting.DrawText(canvas, item.Title, rowRect.Left + 12f, y + 22f,
                textColor, 13f, maxWidth: available);

            // 直接显示「起始 – 截止」区间，跨天时带上两端的日期。
            WidgetPainting.DrawText(canvas, item.RangeText, rowRect.Left + 12f, y + 40f,
            ...
            if (item.IsCompleted)
            {
                // 已完成：标题上加一道删除线。
                WidgetPainting.DrawLine(canvas,
                    rowRect.Left + 12f, y + 18f,
                    rowRect.Left + 12f + WidgetPainting.MeasureText(item.Title, 13f), y + 18f,
                    WidgetTheme.Fade(_theme.TextPrimary, 0x66));
            }
    ...
    protected override void OnPointerPressed(SKPoint point)
    {
        ...
        {
            // 自己跟踪指针位移，而不是交给系统模态循环——后者会卡在循环里，
            // 表现为指针变等待、窗口黏住鼠标（主窗口标题栏踩过这个坑）。
            _dragging = true;
    ...
    protected override void OnPointerReleased(SKPoint point)
    {
        ...
        {
            ScheduleActivated?.Invoke(_items[index]);
        }
    }
